...
void readAcceleration() {
  if (!mpu_ok) { data.a_total = 0.0f; return; }
...
void sendTelemetry() {
  ...
  Serial.printf("TX ID=%u | TOTAL=%.2f ALT=%.2f P=%.2f T=%.2f RH=%.2f | ",
    data.packetID, data.a_total,
...
void logTelemetryToSD() {
  ...
  if (logFile) {
    logFile.printf("%u,%.2f,%.2f,%.2f,%.2f,%.2f,0x%02X\n",
      data.packetID, data.a_total,
...
void setup() {
  ...
  if (!initSD()) {
    ...
    if (!SD_MMC.exists("/telemetry.csv")) {
      ...
      if (logFile) {
        logFile.println("Packet ID,Accel Total,Temp (C),Humidity (%),Pressure (hPa),Altitude (m),Checksum");
