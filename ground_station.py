...
class CanSatDashboard:
    def __init__(self, root):
        ...
        self.accel = tk.StringVar(value="Accel (m/s²): X: 0.0 | Y: 0.0 | Z: 0.0 | Tot: 0.0")
        ...
        with open(self.csv_filename, mode='w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([
                "Packet ID", "Accel X (m/s2)", "Accel Y", "Accel Z", 
                "Accel Total", "Temp (C)", "Humidity (%)", 
    ...
    def read_serial(self):
        # Struct format: < (little endian), H (uint16), 8f (8 floats), B (uint8)
        packet_format = '<H8fB' 
        struct_size = struct.calcsize(packet_format) # Must be exactly 35 bytes
    ...
    def update_ui(self, data):
        ...
        self.accel.set(f"Accel (m/s²): X: {ax:.2f} | Y: {ay:.2f} | Z: {az:.2f} | Tot: {a_tot:.2f}")
        ...
        with open(self.csv_filename, mode='a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([
                pid, f"{ax:.2f}", f"{ay:.2f}", f"{az:.2f}", f"{a_tot:.2f}", 
